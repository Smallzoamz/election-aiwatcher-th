...
export async function simulateMarket() {
    ...
    try {
        ...
        const available = newsList.filter(n => {
            ...
            // Normalize link: Remove query params and trailing slashes for better duplicate detection
            ...
            const inHistory = analyzedHistory.some(h => {
                const hHeadlineKey = (h.headline || '').toLowerCase().replace(/[\s\W]/g, '');
        ...
        const batchImpacts = {};
        const batchSize = analyzedHistory.length < 5 ? 3 : 1;
        const workQueue = [...mainstream.sort((a, b) => new Date(b.pubDate || 0) - new Date(a.pubDate || 0)), ...social].slice(0, batchSize);
