...
async function fetchLiveNewsLocal() {
    ...
    const uniqueNews = recentNews.filter(item => {
        ...
        const normalizedTitle = (item.text || '')
            .toLowerCase()
            .replace(/[\s\n\r\t\u00A0]+/g, '') // Remove all whitespace including non-breaking space
            .replace(/[""''\"\'`\-–—\.,:;!?()[\]{}]/g, '') // Remove punctuation
            .replace(/[ๅๆ็่้๊๋์ํ๎]/g, '') // Remove Thai tone marks for better matching
            .trim();

        if (normalizedTitle.length === 0) return false; // Skip empty titles
            ...
            try {
                ...
                recentPoliticalNews = freshNews
                    .filter(n => {
                        ...
                        const linkKey = n.link ? n.link.split('?')[0] : null; // Remove query params
                        if (seenHeadlines.has(n.text)) return false;
                        if (linkKey && seenHeadlines.has(linkKey)) return false;
                        seenHeadlines.add(n.text);
                        if (linkKey) seenHeadlines.add(linkKey);
                        return true;
                    })
