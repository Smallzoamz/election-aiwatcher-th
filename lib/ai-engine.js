...
const FALLBACK_NIDA_DATA = { 'pp': 34.20, 'bjt': 22.60, 'pt': 16.20, 'dem': 13.20, 'econ': 3.40, 'utn': 2.20, 'pcc': 1.50, 'tst': 1.12, 'srt': 0.50, 'pprp': 0.50, 'cpd': 0.50 };
...
const NEGATIVE_KEYWORDS = [
    ...
    { word: 'ไล่ออก', weight: 2.0, context: 'scandal' },
];

const SENTIMENT_MODIFIERS = [
    { word: 'ปราบ', effect: 'reverse_neg', weight: 1.5 },
    { word: 'ต้าน', effect: 'reverse_neg', weight: 1.5 },
    { word: 'จัดการ', effect: 'reverse_neg', weight: 1.2 },
    { word: 'แฉ', effect: 'expose', weight: 1.0 },
];
...
export async function analyzeSentimentHybrid(text, content = "") {
    ...
    // [NEW] Rule-based sentiment correction for "Anti-Corruption"
    SENTIMENT_MODIFIERS.forEach(mod => {
        if (text.includes(mod.word)) {
            const hasNeg = rule.keywords.some(k => k.type === 'neg' && text.includes(mod.word + k.word));
            if (hasNeg && mod.effect === 'reverse_neg') {
                rule.sentiment = 'pos';
                rule.score = Math.abs(rule.score) || 1.0;
                rule.primaryContext = { context: 'policy', label: 'นโยบาย', weight: 2.0 };
                rule.modelUsed = 'Local Rule Engine (Modified by Context)';
            }
        }
    });
...
export function detectParty(text) {
    const COMMON_PEOPLE_PHRASES = ["ประชาชนเข้าชื่อ", "สิทธิประชาชน", "เสียงประชาชน", "เงินประชาชน", "ภาษีประชาชน", "ประชาชนเดือดร้อน"];

    for (const [id, kw] of Object.entries(PARTY_KEYWORDS)) {
        if (id === 'pp') {
            const hasPartyName = text.includes("พรรคประชาชน");
            const hasSpecificKeywords = kw.slice(1).some(k => text.includes(k)); // exclude "ประชาชน"
            const hasGeneralPeople = text.includes("ประชาชน");
            const isCommonPhrase = COMMON_PEOPLE_PHRASES.some(p => text.includes(p));

            if (isCommonPhrase) continue; // Definitely general people
            if (hasPartyName || hasSpecificKeywords) return id;
            if (hasGeneralPeople && text.length < 50) return id;
            continue;
        }
...
export async function simulateMarket() {
    ...
    try {
        ...
        const available = newsList.filter(n => {
            ...
            // Re-sync logic: If UI history is low, allow "read" news to be re-processed 
            // only for the purpose of populating the UI history [v4.5]
            if (analyzedHistory.length < 5 && !inHistory) {
                return true;
            }
