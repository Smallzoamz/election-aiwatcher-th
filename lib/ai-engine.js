...
const FALLBACK_NIDA_DATA = { 'pp': 28.5, 'bjt': 25.0, 'pt': 12.0, 'pcc': 10.0, 'dem': 8.5, 'utn': 7.5, 'pprp': 2.5, 'cpd': 1.5, 'tst': 1.0, 'srt': 0.8 };
...
const PARTY_KEYWORDS = {
    ...
    'pcc': ["ประชาชาติ", "ทวี สอดส่อง", "วรวีร์", "ซูการ์โน"],
};
...
async function fetchLiveNewsLocal() {
    ...
    await Promise.all(RSS_FEEDS.map(async (feed) => {
        try {
            // First, fetch raw text using standard fetch (mirroring the test script)
    ...
    // [BEST EFFORT FALLBACK] If no fresh news, take top 7 most recent political news
    if (filtered.length === 0 && politicalFallback.length > 0) {
        logDebug("⚠️ No fresh news (48h). Using top 7 most recent political news as fallback.");
        politicalFallback.sort((a, b) => new Date(b.pubDate) - new Date(a.pubDate));
        filtered.push(...politicalFallback.slice(0, 7));
    }
...
export async function simulateMarket() {
    ...
    try {
        ...
        const available = newsList.filter(n => {
            const headlineKey = (n.text || '').toLowerCase().replace(/[\s\W]/g, '');
            const linkKey = (n.link || '').split('?')[0].replace(/\/$/, '').trim();

            const isRead = consumed.has(headlineKey) || (linkKey && consumed.has(linkKey));

            // Also check if it's already in the currently shown analyzedHistory
            const inHistory = analyzedHistory.some(h =>
                (h.headline || '').toLowerCase().replace(/[\s\W]/g, '') === headlineKey
            );
        ...
        if (newsItem) {
            const headlineKey = (newsItem.text || '').toLowerCase().replace(/[\s\W]/g, '');
            ...
            markNewsAsRead(headlineKey, partyId, sentimentResult.sentiment, impact);
            ...
            // STRICT DUPLICATE GUARD: Prevent adding same headline to history
            const isLatestDuplicate = analyzedHistory.length > 0 &&
                (analyzedHistory[0].headline?.toLowerCase().replace(/[\s\W]/g, '') === headlineKey);
        ...
        return {
            ...
            analyzedNews: (newsItem && newsItem.type === 'news') ? analyzedHistory[0] : null,
