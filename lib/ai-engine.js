...
// Weighted keywords with intensity scores
const POSITIVE_KEYWORDS = [
    { word: 'สำเร็จ', weight: 2.0 },
    { word: 'ชนะ', weight: 2.0 },
    { word: 'หนุน', weight: 1.5 },
    { word: 'เห็นชอบ', weight: 1.5 },
    { word: 'ชื่นชม', weight: 1.5 },
    { word: 'ผ่านฉลุย', weight: 1.8 },
    { word: 'ก้าวหน้า', weight: 1.3 },
    { word: 'ฟื้นตัว', weight: 1.5 },
    { word: 'ช่วย', weight: 1.0 },
    { word: 'แก้ปัญหา', weight: 1.5 },
    { word: 'พัฒนา', weight: 1.2 },
    { word: 'สนับสนุน', weight: 1.3 },
    { word: 'นำ', weight: 1.0 },
    { word: 'มั่นใจ', weight: 1.2 },
    { word: 'ประกาศ', weight: 0.5 },
    { word: 'ลุย', weight: 1.0 },
    { word: 'ยก', weight: 0.8 },
];

const NEGATIVE_KEYWORDS = [
    { word: 'ทุจริต', weight: 2.5 },
    { word: 'โกง', weight: 2.5 },
    { word: 'จับกุม', weight: 2.0 },
    { word: 'ค้าน', weight: 1.5 },
    { word: 'เดือด', weight: 1.3 },
    { word: 'สอบ', weight: 1.5 },
    { word: 'วิจารณ์', weight: 1.3 },
    { word: 'ไม่เห็นด้วย', weight: 1.5 },
    { word: 'ล้ม', weight: 1.8 },
    { word: 'ขัดแย้ง', weight: 1.5 },
    { word: 'ประท้วง', weight: 1.8 },
    { word: 'ถล่ม', weight: 1.5 },
    { word: 'โจมตี', weight: 1.5 },
    { word: 'แฉ', weight: 1.8 },
    { word: 'ฉาว', weight: 2.0 },
    { word: 'พัวพัน', weight: 1.5 },
    { word: 'เอี่ยว', weight: 1.5 },
    { word: 'เสียหาย', weight: 1.3 },
];

function analyzeSentiment(text) {
    if (!text) return { sentiment: 'neu', score: 0, keywords: [] };

    let totalScore = 0;
    const matchedKeywords = [];

    POSITIVE_KEYWORDS.forEach(({ word, weight }) => {
        if (text.includes(word)) {
            totalScore += weight;
            matchedKeywords.push({ word, type: 'pos', weight });
        }
    });

    NEGATIVE_KEYWORDS.forEach(({ word, weight }) => {
        if (text.includes(word)) {
            totalScore -= weight;
            matchedKeywords.push({ word, type: 'neg', weight });
        }
    });
    ...
    return { sentiment, score: totalScore, keywords: matchedKeywords };
}
