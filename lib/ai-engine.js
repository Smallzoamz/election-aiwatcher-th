...
// ---------------------------------------------------------
// [REPLACED] Wikipedia PageViews Data is now fetched in real-time
// ---------------------------------------------------------
function getWikiIndex(partyId, trendsData = null) {
    if (trendsData && trendsData[partyId]) {
        // Use real-time normalized score (0.05 - 1.0) scaled to 0-100
        return trendsData[partyId] * 100;
    }
...
// [REMOVED] Old manual getWikiIndex
...
const CONTEXT_LABELS = {
    ...
    'victim': 'ถูกกระทำ', // [NEW] Victim context
};

// ==================== VICTIM/PERPETRATOR CONTEXT DETECTION ====================
// Patterns to detect when a party/candidate is being VICTIMIZED (not the perpetrator)
const VICTIM_PATTERNS = [
    // Pattern: ถูก + [negative action] = victim
    { prefix: 'ถูก', words: ['คุกคาม', 'โจมตี', 'ทำร้าย', 'ข่มขู่', 'กลั่นแกล้ง', 'ใส่ร้าย', 'ทุบ', 'เผา', 'ทำลาย', 'ขู่', 'ปาหิน', 'ปา', 'ด่า', 'ประท้วง'] },
    // Pattern: โดน + [negative action] = victim
    { prefix: 'โดน', words: ['คุกคาม', 'โจมตี', 'ทำร้าย', 'ข่มขู่', 'กลั่นแกล้ง', 'ใส่ร้าย', 'ทุบ', 'เผา', 'ทำลาย', 'ด่า', 'ปาหิน', 'ปา', 'ประท้วง'] },
    // Pattern: Exposing/Whistleblowing = neutral/positive for the whistleblower
    ...
    // Direct victim indicators (contains pattern)
    ...
    { contains: 'เดือดจัด', isVictim: true }, // Usually describes victim's frustration
];

/**
 * Detect if the news indicates the party/candidate is a VICTIM (not perpetrator)
 * @param {string} text - News headline
 * @returns {{ isVictim: boolean, isWhistleblower: boolean, matchedPattern: string|null }}
 */
function detectVictimContext(text) {
    ...
    for (const pattern of VICTIM_PATTERNS) {
        ...
        // Check direct victim patterns (contains)
...
function calculateAmbiguityScore(text, score, posCount, negCount) {
    // [NEW] 0. Check for victim context - ALWAYS escalate to Gemini for better understanding
    const victimContext = detectVictimContext(text);
    if (victimContext.isVictim || victimContext.isWhistleblower) {
        const contextType = victimContext.isVictim ? 'Victim' : 'Whistleblower';
        console.log(`🛡️ ${contextType} pattern detected: "${victimContext.matchedPattern}". Escalating to Gemini for context analysis.`);
        return 0.9; // Force escalation to Gemini
    }
...
async function fetchLiveNewsLocal() {
    ...
    // [FIX] Remove duplicate headlines (strict matching - improved)
    const seenHeadlines = new Set();
    const seenLinks = new Set();
    const uniqueNews = recentNews.filter(item => {
        // Normalize: lowercase, remove ALL whitespace and special chars, use FULL title
        const normalizedTitle = (item.text || '')
            .toLowerCase()
            .replace(/[\s\n\r\t\u00A0]+/g, '') // Remove all whitespace
            .replace(/[""''\"\'`\-–—\.,:;!?()[\]{}]/g, '') // Remove punctuation
            .replace(/[ๅๆ็่้๊๋์ํ๎]/g, '') // Remove Thai tone marks
            .trim();

        // Normalize Link: strip query params and trailing slash
        const normalizedLink = (item.link || '').split('?')[0].replace(/\/$/, '').trim();

        if (normalizedTitle.length === 0) return false;

        // Check both headline and link
        if (seenHeadlines.has(normalizedTitle) || (normalizedLink && seenLinks.has(normalizedLink))) {
            return false;
        }
...
export async function simulateMarket() {
    ...
    try {
        ...
        let availableNews = newsList.filter(n => {
            // [STRICT] Normalize exactly as we do for marking
            const headlineKey = (n.text || '')
                .toLowerCase()
                .replace(/[\s\n\r\t\u00A0]+/g, '')
                .replace(/["\'\`\-–—\.,:;!?()\[\]{}]/g, '')
                .replace(/[ๅๆ็่้๊๋์ํ๎]/g, '')
                .trim();

            const linkKey = (n.link || '').split('?')[0].replace(/\/$/, '').trim();

            return !consumedNewsLinks.has(headlineKey) && (!linkKey || !consumedNewsLinks.has(linkKey));
        });
        ...
        if (noNewNewsAvailable && cachedRecentNews.length > 0 && (now - lastNewsCacheTime) < NEWS_CACHE_DURATION) {
            ...
            try {
                ...
                recentPoliticalNews = freshNews
                    .filter(n => {
                        // Deduplicate: skip if headline OR link already seen
                        const normalizedTitle = (n.text || '')
                            .toLowerCase()
                            .replace(/[\s\n\r\t\u00A0]+/g, '')
                            .replace(/[""''\"\'`\-–—\.,:;!?()[\]{}]/g, '')
                            .replace(/[ๅๆ็่้๊๋์ํ๎]/g, '')
                        ...
                        const normalizedLink = (n.link || '').split('?')[0].replace(/\/$/, '').trim();

                        if (seenHeadlines.has(normalizedTitle)) return false;
                        if (normalizedLink && seenLinks.has(normalizedLink)) return false;
        ...
        if (availableNews.length > 0) {
            // [FIX] Chronological Timeline: Sort by date ASC (Oldest First) 
            // This ensures we process news in order, and history.reverse() shows them correctly
            availableNews.sort((a, b) => {
                const dateA = a.pubDate ? new Date(a.pubDate).getTime() : 0;
                const dateB = b.pubDate ? new Date(b.pubDate).getTime() : 0;
                return dateA - dateB;
            });

            // Pick the OLDEST unconsumed news first to maintain narrative timeline
            newsItem = availableNews[0];
            ...
            // [STRICT] Mark BOTH normalized headline and Link as read
            const headlineKey = (newsItem.text || '')
                .toLowerCase()
                .replace(/[\s\n\r\t\u00A0]+/g, '')
                .replace(/["\'\`\-–—\.,:;!?()\[\]{}]/g, '')
                .replace(/[ๅๆ็่้๊๋์ํ๎]/g, '')
            ...
            const linkKey = (newsItem.link || '').split('?')[0].replace(/\/$/, '').trim();

            // Determine the impacted party for marking
            const impactedParty = marketState.find(p => p.id === partyId);

            // Catch any errors in marking to prevent engine crash
            const markErrors = (err) => console.error('Failed to mark news as read:', err);
            markNewsAsRead(headlineKey, impactedParty?.id, sentimentResult.sentiment, impact).catch(markErrors);
            if (linkKey) markNewsAsRead(linkKey, impactedParty?.id, sentimentResult.sentiment, impact).catch(markErrors);
            if (newsItem.text !== headlineKey) {
                markNewsAsRead(newsItem.text, impactedParty?.id, sentimentResult.sentiment, impact).catch(markErrors);
            }
        }
        ...
        const listScores = marketState.map(p => {
            ...
            const wikiIndex = getWikiIndex(p.id, trendsData); // Pass real-time data

            // [STRICT] Papa's Formula (Ref: Image uploaded)
            // AI Hybrid = (Sentiment * 75%) + (Wikipedia Index * 25%)
            // Final List Score = (NIDA Poll * 40%) + (AI Hybrid * 60%)

            const aiHybrid = (sentimentScore * 0.75) + (wikiIndex * 0.25);
            const hybridScore = (nida * 0.40) + (aiHybrid * 0.60);

            return {
                id: p.id,
                val: hybridScore,
                _debug: { nida, sentiment: sentimentScore, wiki: wikiIndex, aiHybrid }
            };
        });

        const constScores = marketState.map(p => {
            const nida = nidaData[p.id] || 0.5;
            const aiSentiment = p.score ?? p.baseScore;

            // [STRICT] Papa's Formula (Ref: Image uploaded)
            // Final Constituency Score = (NIDA Poll * 70%) + (AI Sentiment * 30%)
            const hybridScore = (nida * 0.70) + (aiSentiment * 0.30);

            return { id: p.id, val: hybridScore };
        });
        ...
        marketState = marketState.map(p => {
            ...
            return {
                ...
                wikiIndex: getWikiIndex(p.id, trendsData).toFixed(1), // Expose real-time index
                ...
                hybridScore: listRaw.toFixed(2) // List Hybrid (40/60)
            };
        });
        ...
        return {
            ...
            analyzedNews: {
                ...
                link: newsItem.link,
                pubDate: newsItem.pubDate ? new Date(newsItem.pubDate).toISOString() : new Date().toISOString(), // [FIX] Ensure valid date
