...
// [NEW] Clear old consumed news (keep only last 100)
export function trimConsumedNewsCache() {
    if (consumedNewsCache.size > 100) {
        const oldSize = consumedNewsCache.size;
        const arr = Array.from(consumedNewsCache);
        consumedNewsCache = new Set(arr.slice(-100)); // Keep last 100
        console.log(`🔄 Trimmed consumed news: ${oldSize} → ${consumedNewsCache.size}`);
    }
}
    ...
    try {
        const { error } = await supabase
            ...
            .upsert({
                news_key: newsKey,
                party_id: partyId,
                // sentiment and impact columns may not exist in schema - store in memory only
