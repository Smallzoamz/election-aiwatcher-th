...
/**
 * Analyze a news headline and full content using Gemini 2.0 Flash
 * @param {string} headline - The news headline to analyze
 * @param {string} content - The full article content for context
 * @returns {Promise<{sentiment: string, score: number, confidence: number, reason: string, party: string, modelUsed: string, isGemini: boolean}>}
 */
export async function analyzeWithGemini(headline, content = "") {
    ...
    try {
        const prompt = `วิเคราะห์หัวข้อข่าวและเนื้อหาการเมืองไทยนี้อย่างละเอียด:
หัวข้อ: "${headline}"
เนื้อหา: "${content.substring(0, 1500)}"

ตอบเป็น JSON เท่านั้น ในรูปแบบ:
{"sentiment": "pos|neg|neu", "score": -1.0 ถึง 1.0, "confidence": 0 ถึง 100, "reason": "เหตุผลสั้นๆ", "party": "ชื่อพรรคที่เกี่ยวข้องหรือ null"}

ตัวอย่าง: {"sentiment": "neg", "score": -0.7, "confidence": 95, "reason": "มีการกล่าวหาทุจริต", "party": "เพื่อไทย"}`;
        ...
        return {
            ...
            confidence: parseInt(parsed.confidence) || 90,
            reason: parsed.reason || 'Gemini วิเคราะห์โดยละเอียด',
